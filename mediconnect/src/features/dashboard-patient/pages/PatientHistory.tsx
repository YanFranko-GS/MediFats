...
export default function PatientHistory() {
  ...
  return (
    <>
      <Helmet><title>Historial Médico – MediConnect</title></Helmet>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total invertido', value: `$${totalSpent.toLocaleString()}`, color: 'text-amber-500' },
        ].map((s, i) => (
                  ...
                  {appts.map((appt, i) => {
                    ...
                    return (
                      <motion.div key={appt.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                        ...
                        {/* Card */}
                        <div className="flex-1 card p-4 flex items-center gap-4">
                          ...
                          {appt.status === 'completed' && (
                            <Link to={`/doctor/${appt.doctorId}`}
                              className="shrink-0 p-2 rounded-lg text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/20 transition-colors"
                              title="Dejar reseña">
