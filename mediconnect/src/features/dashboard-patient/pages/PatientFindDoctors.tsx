...
export default function PatientFindDoctors() {
  ...
  const apply = () => setFilters(f => ({ ...f, query: localQ }));
  ...
  return (
    <>
      <Helmet><title>Buscar Médicos – MediConnect</title></Helmet>
      ...
      <div className="card p-4 mb-6">
        <div className="flex gap-2 mb-3">
          <div className="flex-1 flex items-center gap-2 bg-surface-50 dark:bg-slate-800 border border-surface-200 dark:border-slate-700 rounded-xl px-4 py-2.5">
          ...
          <button onClick={apply} className="btn-primary px-5 py-2.5 text-sm">Buscar</button>
        ...
        {showFilters && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-3 border-t border-surface-100 dark:border-slate-800">
            <select value={filters.specialty || ''} onChange={e => setFilters(f => ({ ...f, specialty: e.target.value }))} className="input-base text-xs py-2 col-span-2">
              <option value="">{t('patientDashboard.specialty')}</option>
              {(specialties || []).map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
            </select>
            <select value={filters.minRating || ''} onChange={e => setFilters(f => ({ ...f, minRating: e.target.value ? Number(e.target.value) : undefined }))} className="input-base text-xs py-2">
              <option value="">Rating mín.</option>
              {[4.5, 4.0, 3.5].map(r => <option key={r} value={r}>★ {r}+</option>)}
            </select>
            <select value={filters.maxPrice || ''} onChange={e => setFilters(f => ({ ...f, maxPrice: e.target.value ? Number(e.target.value) : undefined }))} className="input-base text-xs py-2">
              <option value="">Precio máx.</option>
              {[100, 150, 200, 300].map(p => <option key={p} value={p}>≤ ${p}</option>)}
            </select>
            <select value={filters.mode || ''} onChange={e => setFilters(f => ({ ...f, mode: e.target.value as any || undefined }))} className="input-base text-xs py-2">
              <option value="">Modalidad</option>
              <option value="in-person">Presencial</option>
              <option value="video">Video</option>
              <option value="chat">Chat</option>
            </select>
            <select value={filters.sortBy || 'rating'} onChange={e => setFilters(f => ({ ...f, sortBy: e.target.value as any }))} className="input-base text-xs py-2">
                ...
                <Link to={`/doctor/${doc.id}`} className="card card-hover block p-5 group relative">
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    <div className="relative">
                      <img src={doc.avatar} alt={doc.name} className="w-12 h-12 rounded-xl bg-primary-50 object-cover" />
