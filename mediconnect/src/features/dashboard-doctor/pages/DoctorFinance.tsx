    ...
    <>
      ...
      <div className="grid lg:grid-cols-2 gap-5 mb-5">
        <div className="card p-5">
          ...
          <p className="text-xs text-slate-400 mb-4">Miles de USD · {period} meses</p>
          {isLoading ? <Skeleton className="h-48" /> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} barSize={18}>
                ...
                <Bar dataKey="ingresos" name="Ingresos (k$)" fill="#22c55e" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {isLoading ? <Skeleton className="h-48" /> : (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="ticket" name="Ticket ($)" stroke="#f59e0b" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      {currentMonth && prevMonth && (
        <div className="card p-5 mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: 'Ingresos', curr: `$${currentMonth.ingresos}k`, prev: `$${prevMonth.ingresos}k`, positive: currentMonth.ingresos >= prevMonth.ingresos },
              { label: 'Consultas', curr: currentMonth.citas, prev: prevMonth.citas, positive: currentMonth.citas >= prevMonth.citas },
              { label: 'Ticket promedio', curr: `$${currentMonth.ticket}`, prev: `$${prevMonth.ticket}`, positive: currentMonth.ticket >= prevMonth.ticket },
