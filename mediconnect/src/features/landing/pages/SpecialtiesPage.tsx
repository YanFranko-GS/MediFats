...
const HERO_BADGES = [
  [<Users className="h-4 w-4 text-primary-600"/>, 'Médicos especialistas'],
  [<Heart className="h-4 w-4 text-primary-600"/>, 'Atención personalizada'],
  [<Calendar className="h-4 w-4 text-primary-600"/>, 'Reservas en minutos'],
  [<Shield className="h-4 w-4 text-primary-600"/>, 'Cuidado confiable'],
];
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
              <span className="inline-flex items-center gap-1.5 text-primary-600 text-xs font-bold uppercase tracking-widest mb-3">
                <span className="w-3 h-0.5 bg-primary-600"/> +20 especialidades médicas
              </span>
              <h1 className="text-3xl lg:text-4xl font-heading font-bold text-slate-900 dark:text-white leading-tight mb-3">
                Encuentra la<br />
                <span className="text-primary-600">especialidad ideal</span><br />
                para ti
              </h1>
              <p className="text-slate-500 dark:text-slate-400 mb-5 leading-relaxed max-w-md">
...
>
  <div className="relative w-[500px] h-[500px]">
    ...
    <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-6">
      ...
      {/* Card 1 */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg px-5 py-4 w-[220px]">
      ...
      <section className="bg-primary-800 py-8">
        <div className="max-w-5xl mx-auto px-6">
          <h2 className="text-white font-heading font-bold text-lg mb-4">Encuentra tu especialidad ideal</h2>
          <div className="flex flex-wrap gap-3 items-center bg-white dark:bg-slate-900 rounded-xl p-2 shadow-lg">
            <div className="flex-1 flex items-center gap-2 px-3 min-w-48 border-r border-surface-200 dark:border-slate-700">
            ...
            <div className="px-3 border-r border-surface-200 dark:border-slate-700">
              <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
                className="text-sm text-slate-600 dark:text-slate-300 bg-transparent focus:outline-none py-1">
            ...
            <button className="bg-primary-600 hover:bg-primary-700 text-white font-semibold px-6 py-2 rounded-lg text-sm transition-colors">
              Buscar
            </button>
          </div>
          {/* Popular tags */}
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className="text-primary-200 text-xs font-medium">Popular:</span>
            {POPULAR_TAGS.map(tag => (
              <button key={tag} onClick={() => setQuery(tag)}
                className={cn('px-3 py-1 rounded-full border text-xs transition-colors',
                  query === tag ? 'bg-white text-primary-700 border-white' : 'border-primary-400/50 text-primary-100 hover:bg-primary-600/50')}>
                {tag}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <motion.div {...fadeUp}>
              <span className="text-primary-600 text-xs font-bold uppercase tracking-widest flex items-center gap-1.5 mb-1">
                <span className="w-3 h-0.5 bg-primary-600"/> ESPECIALIDADES MÉDICAS
              </span>
            ...
            <div className="flex gap-2 shrink-0">
              <button className="px-4 py-2 rounded-xl border border-surface-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-400 hover:bg-surface-50 dark:hover:bg-slate-800 transition-colors">
                Ver todas las especialidades
              </button>
              <button className="p-2 rounded-xl border border-surface-200 dark:border-slate-700 text-slate-500 hover:bg-surface-50 dark:hover:bg-slate-800 transition-colors">
              ...
              {filtered.map((sp, i) => (
                <motion.div key={sp}
                  ...
                  <Link to={`/doctors?specialty=${encodeURIComponent(sp)}`}
                    className="flex flex-col p-5 rounded-2xl border border-surface-200 dark:border-slate-800 hover:border-primary-300 hover:shadow-md transition-all bg-white dark:bg-slate-900 group h-full">
                    {/* Icon */}
                    <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center mb-3 transition-transform group-hover:scale-110', SPECIALTY_COLORS[sp] || 'bg-primary-50 text-primary-600')}>
                      {SPECIALTY_ICONS[sp] || <Stethoscope className="h-5 w-5"/>}
                    </div>
                    {/* Name */}
                    <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm mb-1 leading-tight">{sp}</p>
                    {/* Description */}
                    <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed mb-3 flex-1">
                      {SPECIALTY_DESC[sp] || 'Especialistas certificados y verificados.'}
                    </p>
                    {/* Footer */}
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-xs text-slate-500">{SPECIALTY_DOCS[sp] || 4} especialistas</span>
  ...
  <div className="max-w-screen-2xl mx-auto px-6">
    <div className="bg-white dark:bg-slate-900 border border-primary-100 dark:border-slate-800 rounded-3xl px-8 py-8 overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-12">
        ...
        <div className="hidden lg:flex items-center justify-center relative flex-shrink-0 w-[280px]">
          ...
          <div className="absolute w-56 h-56 rounded-full bg-primary-100 dark:bg-primary-900/20" />
        ...
        <div className="flex-1 max-w-[480px]">
          ...
          <h3 className="text-4xl font-bold text-primary-600 mb-4 leading-tight">
        ...
        <div className="hidden lg:flex items-center gap-8 flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-primary-50 dark:bg-primary-950/20 flex items-center justify-center">
              <Users className="h-6 w-6 text-primary-600" />
            </div>

            <span className="font-medium text-slate-700 dark:text-slate-300 max-w-[120px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="font-medium text-slate-700 dark:text-slate-300 max-w-[140px]">
      ...
      {/* ── TRUST BAR ─────────────────────────────────────────────────────── */}
      <section className="py-8 bg-white dark:bg-slate-900 border-t border-surface-100 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon:<Clock className="h-5 w-5 text-primary-600"/>, t:'Atención rápida', d:'Reservas en minutos' },
              { icon:<Users className="h-5 w-5 text-primary-600"/>, t:'Médicos certificados', d:'Profesionales de confianza' },
              { icon:<Shield className="h-5 w-5 text-primary-600"/>, t:'Seguridad', d:'Tus datos están protegidos' },
              { icon:<Star className="h-5 w-5 text-amber-500"/>, t:'Calidad garantizada', d:'Excelencia en cada atención' },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-primary-600 dark:text-primary-400">{item.t}</p>
