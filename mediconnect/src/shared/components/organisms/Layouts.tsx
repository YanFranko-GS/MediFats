...
function Footer() {
  ...
  return (
    <footer className="bg-white dark:bg-slate-950 border-t border-surface-200 dark:border-slate-800 mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div>
            <h3 className="font-heading font-bold text-slate-800 dark:text-slate-100 mb-3">
              CLÍNICA <span className="text-primary-600">FAST</span>
            </h3>
          ...
          {[
            ...
            <div key={title}>
              ...
              <ul className="space-y-2">
                {links.map((l) => (
                  <li key={l.label}>
                    <Link to={l.to} className="text-sm text-slate-500 dark:text-slate-400 hover:text-primary-600 transition-colors">{l.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-t border-surface-200 dark:border-slate-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-400">© 2025 CLÍNICA FAST. {t('footer.rights')}.</p>
          <p className="text-xs text-slate-400 flex items-center gap-1">{t('footer.madeWith')} <Heart className="h-3 w-3 fill-red-400 text-red-400"/> {t('footer.forPeru')}</p>
        </div>
      </div>
    </footer>
  );
