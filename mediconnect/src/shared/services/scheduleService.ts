...
const ALL = APPOINTMENTS as unknown as Appointment[];
...
export const scheduleService = {
  async getByDay(doctorUserId: string, date: string) {
    return (await apiClient(() => {
      ...
      return ALL.filter(a => a.doctorId === docId && a.date.startsWith(date))
  ...
  async getTodayAppointments(doctorUserId: string) {
    return (await apiClient(() => {
      ...
      return ALL.filter(a => a.doctorId === docId && a.date.startsWith(today))
        .sort((a, b) => a.time.localeCompare(b.time));
