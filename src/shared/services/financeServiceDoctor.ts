...
export const financeServiceDoctor = {
  ...
  async getKPIs(doctorId = 'doc-001') {
    return (await apiClient(() => {
      ...
      // Group by patient to derive real, distinct "new" vs "recurring" counts
      // instead of reusing the same total for both labels.
      const firstVisit = new Map<string, Date>();
      const visitCount = new Map<string, number>();
      docAppts.forEach(a => {
        const d = new Date(a.date);
        visitCount.set(a.patientId, (visitCount.get(a.patientId) || 0) + 1);
        const existing = firstVisit.get(a.patientId);
        if (!existing || d < existing) firstVisit.set(a.patientId, d);
      });
      const now = new Date();
      const newPatients = [...firstVisit.values()].filter(
        d => d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()
      ).length;
      const recurringPatients = [...visitCount.values()].filter(c => c > 1).length;
