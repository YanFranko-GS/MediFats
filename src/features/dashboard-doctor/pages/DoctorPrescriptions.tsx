...
export default function DoctorPrescriptions() {
  ...
  // Arriving from a consultation's "Emitir receta" quick action: open the
  // form pre-filled with that patient instead of making the doctor pick again.
  useEffect(() => {
                ...
                <motion.tr key={rx.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.03}}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {rx.patientAvatar && <AvatarImg src={rx.patientAvatar} alt={rx.patientName} className="w-7 h-7 rounded-full object-cover bg-primary-50 shrink-0"/>}
