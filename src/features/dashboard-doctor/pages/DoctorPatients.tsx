...
import { Search, MessageCircle, Calendar, Download, Eye, UserCheck, UserX, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { Skeleton, Badge, AvatarImg } from '../../../shared/components/atoms/index';
                  ...
                  <motion.tr key={p.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.03}}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <AvatarImg src={p.avatar} alt={p.name} className="w-9 h-9 rounded-full object-cover bg-primary-50 shrink-0"/>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Link to={`/dashboard/doctor/messages?patient=${p.id}`} title="Enviar mensaje" className="p-1.5 rounded-lg hover:bg-surface-100 dark:hover:bg-slate-700 text-slate-500 hover:text-primary-600 transition-colors"><MessageCircle className="h-4 w-4"/></Link>
          ...
          <div className="px-5 py-4 border-t border-surface-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                className="w-8 h-8 rounded-lg text-xs font-medium border border-surface-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-40 hover:bg-surface-100 dark:hover:bg-slate-800 transition-colors flex items-center justify-center">
                <ChevronLeft className="h-3.5 w-3.5" />
              </button>
              {(() => {
                // Show a sliding window of up to 5 page numbers centered on the current page
                const windowSize = 5;
