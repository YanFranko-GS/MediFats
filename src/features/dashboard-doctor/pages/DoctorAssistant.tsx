...
export default function DoctorAssistant() {
  ...
  const buildContextualResponse = (msg: string): string | null => {
    ...
    if (lower.includes('ingreso') || lower.includes('finanzas') || lower.includes('dinero')) {
      return `Este mes llevas **${formatCurrency(kpis?.monthRevenue ?? 0)}** en ingresos con **${kpis?.monthConsultations ?? 0} consultas** realizadas. Tu ticket promedio es **${formatCurrency(kpis?.avgTicket ?? 0)}**. ${(kpis?.monthRevenueChange ?? 0) >= 0 ? `📈 Crecimiento del ${kpis?.monthRevenueChange}% vs mes anterior.` : `📉 Caída del ${Math.abs(kpis?.monthRevenueChange ?? 0)}% vs mes anterior.`}`;
    }
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                { label: 'Ingresos del mes', value: formatCurrency(kpis?.monthRevenue || 0) },
              ].map(({ label, value }) => (
