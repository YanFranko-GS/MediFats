...
import { useDoctorFinanceKPIs, useDoctorFinanceMonthly, usePendingRequests, useDoctorUnreadCount, DOCTOR_ID } from '../../../shared/hooks/useDoctor';
...
import { Skeleton, SkeletonRow, AvatarImg } from '../../../shared/components/atoms/index';
...
export default function DoctorHome() {
  ...
  const { data: reviewData } = useDoctorReviews(DOCTOR_ID, 1);
  ...
  const upcoming = (appointments || []).filter(a => {
    ...
    return dateStr >= format(now, 'yyyy-MM-dd');
  }).sort((a, b) => {
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          { icon: <Users className="h-5 w-5" />, label: t('dashboard.totalPatients'), value: uniquePatientCount, color: 'bg-teal-50 text-teal-600 dark:bg-teal-950/30 dark:text-teal-400', to: '/dashboard/doctor/patients', change: 12.5 },
          ...
          { icon: <UserPlus className="h-5 w-5" />, label: t('dashboard.newPatients'), value: kpis?.newPatients ?? 0, color: 'bg-blue-50 text-blue-600 dark:bg-blue-950/30 dark:text-blue-400', to: '/dashboard/doctor/patients' },
          { icon: <RefreshCw className="h-5 w-5" />, label: t('dashboard.recurring'), value: kpis?.recurringPatients ?? 0, color: 'bg-purple-50 text-purple-600 dark:bg-purple-950/30 dark:text-purple-400', to: '/dashboard/doctor/patients' },
            ...
            <div className="divide-y divide-surface-100 dark:divide-slate-800 max-h-[280px] overflow-y-auto">
              {upcoming.map(appt => (
                <div key={appt.id} className="flex items-center gap-3 px-4 py-3 hover:bg-surface-50 dark:hover:bg-slate-800/50 transition-colors">
                  <AvatarImg src={appt.patientAvatar} alt={appt.patientName} className="w-8 h-8 rounded-full object-cover bg-primary-50 shrink-0" />
              ...
              <div className="space-y-2">
                {(pendingReqs as unknown as any[] || []).slice(0, 3).map((req: any) => (
                  <div key={req.id} className="flex items-center gap-2 p-2 bg-amber-50 dark:bg-amber-950/20 rounded-lg">
                    <AvatarImg src={req.patientAvatar} alt={req.patientName} className="w-7 h-7 rounded-full object-cover bg-primary-50 shrink-0" />
                    <div className="flex-1 min-w-0">
      ...
      <div className="grid lg:grid-cols-2 gap-5 mb-5">
        ...
        <div className="card p-5">
          ...
          <p className="text-xs text-slate-400 mb-4">Miles de Soles</p>
          {!chartData.length ? <Skeleton className="h-48" /> : (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={chartData} barSize={18}>
                ...
                <Bar dataKey="ingresos" name="Ingresos (k S/)" fill="#f59e0b" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
