...
export default function DoctorSchedule() {
  ...
  const { consultationDuration, setConsultationDuration, workingHours, setWorkingHours, toggleWorkingDay, blockSlot, unblockSlot, isBlocked } = useDoctorStore();
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {['Lun','Mar','Mié','Jue','Vie','Sáb'].map((d,i)=>{
                const dayIndex = i + 1; // JS Date.getDay(): Mon=1 ... Sat=6
                const active = workingHours.days.includes(dayIndex);
                return (
                  <button key={d} type="button" onClick={() => toggleWorkingDay(dayIndex)}
                    className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all',
                      active ? 'bg-primary-600 text-white border-primary-600' : 'border-surface-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-primary-300')}>
                    {d}
                  </button>
                );
              })}
            </div>
            <p className="text-xs text-slate-400 mt-2">Los pacientes solo podrán reservar citas en estos días.</p>
          </div>
        </div>
      </Modal>
    </>
