...
export default function DoctorReviews() {
  const { data: reviewData, isLoading } = useDoctorReviews(DOCTOR_ID, 1);
          ...
          reviews.map((r: any, i: number) => (
            <motion.div key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-start gap-4">
                <AvatarImg src={r.patientAvatar} alt={r.patientName} className="w-10 h-10 rounded-full object-cover bg-primary-50 shrink-0" />
