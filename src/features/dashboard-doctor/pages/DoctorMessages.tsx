...
export default function DoctorMessages() {
  ...
  // Auto-select conversation when arriving with ?patient=<id> (e.g. from the
  // patients list "Enviar mensaje" action)
  const requestedPatient = searchParams.get('patient');
  useEffect(() => {
    if (!requestedPatient || !conversations) return;
    const match = (conversations as any[]).find((c: any) => c.contactId === requestedPatient);
    if (match) {
      setActiveConv(match.id);
    } else {
      toast.info('Aún no tienes una conversación con este paciente');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestedPatient, conversations]);
