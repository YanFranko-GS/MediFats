...
import { Spinner, AvatarImg } from '../../../shared/components/atoms/index';
...
export default function PatientMessages() {
  ...
  // Auto-select conversation when arriving with ?doctor=<id> (e.g. from Favorites "Mensaje" button)
  const requestedDoctor = searchParams.get('doctor');
  useEffect(() => {
    if (!requestedDoctor || !conversations) return;
    const match = (conversations as any[]).find((c: any) => c.contactId === requestedDoctor);
    if (match) {
      setActiveConv(match.id);
    } else {
      toast.info('Aún no tienes una conversación con este médico', { description: 'Podrás escribirle una vez tengas una cita confirmada.' });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestedDoctor, conversations]);
            ...
            {loadingConvs ? (
              ...
              <button key={conv.id} onClick={() => setActiveConv(conv.id)}
                ...
                <div className="relative shrink-0">
                  <AvatarImg src={conv.contactAvatar} alt={conv.contactName} className="w-10 h-10 rounded-full bg-primary-50 object-cover" />
          ...
          {!activeConv ? (
            ...
            <>
              ...
              <div className="px-5 py-3 border-b border-surface-200 dark:border-slate-800 flex items-center gap-3 shrink-0">
                {activeConvObj && (<>
                  <AvatarImg src={activeConvObj.contactAvatar} alt={activeConvObj.contactName} className="w-9 h-9 rounded-full bg-primary-50 object-cover" />
                  <div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-surface-50 dark:bg-slate-950">
                ...
                (messages || []).map((msg: any) => (
                  <div key={msg.id} className={cn('flex gap-2 max-w-[75%]', msg.role === 'user' ? 'ml-auto flex-row-reverse' : '')}>
                    <AvatarImg src={msg.senderAvatar} alt={msg.senderName} className="w-7 h-7 rounded-full bg-primary-50 object-cover shrink-0 mt-0.5" />
