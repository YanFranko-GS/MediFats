...
function useMyReviews() {
  ...
  const reviews = completed.map((a: any, i: number) => {
    ...
    return {
      ...
      date: written?.date ?? a.date,
      writtenByPatient: !!written,
    };
  });
...
export default function PatientReviews() {
  ...
  const { addOrUpdateReview, removeReview } = usePatientStore();
  ...
  const allReviews = reviews.filter(r => !deletedIds.includes(r.id));

  const openEdit = (r: any) => { setEditTarget(r); setEditRating(r.rating); setEditComment(r.comment); };

  const handleSaveEdit = () => {
    addOrUpdateReview({
      id: editTarget.id.startsWith('rev-') && !editTarget.writtenByPatient ? `rev-${editTarget.appointmentId}` : editTarget.id,
  ...
  const handleDelete = () => {
    removeReview(deleteTarget.appointmentId);
        ...
        <div className="space-y-4">
          {allReviews.map((rev, i) => (
            <motion.div key={rev.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <AvatarImg src={rev.doctorAvatar} alt={rev.doctorName} className="w-12 h-12 rounded-xl bg-primary-50 object-cover shrink-0" />
        ...
        {editTarget && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-surface-50 dark:bg-slate-800 rounded-xl">
              <AvatarImg src={editTarget.doctorAvatar} alt={editTarget.doctorName} className="w-10 h-10 rounded-xl object-cover bg-primary-50" />
