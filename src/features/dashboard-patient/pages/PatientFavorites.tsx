            ...
            <motion.div key={doc.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
              <div className="card p-5 flex flex-col gap-4">
                <div className="flex items-start gap-3">
                  <AvatarImg src={doc.avatar} alt={doc.name} className="w-14 h-14 rounded-xl object-cover bg-primary-50" />
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-surface-100 dark:border-slate-800">
                  ...
                  <Link to={`/dashboard/patient/messages?doctor=${doc.id}`} className="btn-outline text-xs py-2 text-center flex items-center justify-center gap-1">
