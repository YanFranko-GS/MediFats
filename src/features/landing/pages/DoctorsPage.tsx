...
const TRUST_ITEMS = [
  { icon: <Shield className="h-5 w-5 text-clinical-700" />, t: 'Perfiles verificados', d: 'Todos nuestros médicos pasan por un riguroso proceso.' },
  { icon: <Calendar className="h-5 w-5 text-clinical-700" />, t: 'Información detallada', d: 'Conoce su experiencia, estudios y opiniones de pacientes.' },
  { icon: <Users className="h-5 w-5 text-clinical-700" />, t: 'Reservas seguras', d: 'Tu información está protegida en todo momento.' },
  { icon: <Heart className="h-5 w-5 text-clinical-700" />, t: 'Atención de calidad', d: 'Médicos comprometidos con tu bienestar.' },
];
...
export default function DoctorsPage() {
  ...
  return (
    <>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-14 lg:py-20">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
              <span className="eyebrow mb-4">+100 médicos certificados</span>
              <h1 className="font-display text-4xl lg:text-[3.2rem] font-semibold text-ink-900 dark:text-white leading-[1.08] tracking-tight mb-4">
                Encuentra al<br />
                <span className="italic text-clinical-700 dark:text-clinical-300">especialista ideal</span><br />
                para ti
              </h1>
              <p className="text-ink-500 dark:text-slate-400 mb-5 leading-relaxed max-w-sm text-lg">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  [<Calendar className="h-4 w-4 text-vital-500" />, 'Reserva en minutos'],
                  [<Shield className="h-4 w-4 text-vital-500" />, 'Seguridad y confianza'],
                ].map(([icon, lbl], i) => (
                ...
                <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-6">
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-clinical px-5 py-4 w-[220px] border border-clinical-100/60 dark:border-slate-700">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-2xl font-display font-semibold text-ink-900 dark:text-white leading-none">
                          100%
                        </p>
      ...
      {/* SEARCH BAR */}
      <section className="bg-gradient-to-r from-clinical-800 to-clinical-600 py-8 sticky top-16 z-20">
        <div className="max-w-5xl mx-auto px-6">
          <div className="flex flex-wrap gap-3 items-center bg-white dark:bg-slate-900 rounded-xl p-2 shadow-clinical">
            ...
            <div className="h-8 w-px bg-mist-200 dark:bg-slate-700 hidden sm:block" />
            <select value={filters.specialty} onChange={e => setFilters(f => ({ ...f, specialty: e.target.value }))}
              className="text-sm text-ink-700 dark:text-slate-300 bg-transparent focus:outline-none px-2 py-1">
              <option value="">Todas las especialidades</option>
              {(specialties as unknown as any[] || []).map((s: any) => <option key={s.id} value={s.name}>{s.name}</option>)}
            </select>
            <div className="h-8 w-px bg-mist-200 dark:bg-slate-700 hidden sm:block" />
            <select className="text-sm text-ink-700 dark:text-slate-300 bg-transparent focus:outline-none px-2 py-1">
              <option>Todas las ubicaciones</option>
              <option>Lima</option><option>Arequipa</option><option>Trujillo</option>
            </select>
            <button onClick={applyQuery}
              className="bg-vital-500 hover:bg-vital-600 text-white font-semibold px-6 py-2 rounded-lg text-sm transition-colors shadow-vital">
              Buscar
            </button>
          </div>
          {/* Popular tags */}
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className="text-clinical-200 text-xs font-medium">Búsquedas populares:</span>
            {POPULAR_TAGS.map(tag => (
              <button key={tag} onClick={() => setFilters(f => ({ ...f, specialty: tag }))}
                className="px-3 py-1 rounded-full border border-white/30 text-white/90 text-xs hover:bg-white/10 transition-colors">
                {tag}
              </button>
            ))}
          </div>
      ...
      {/* DOCTORS LIST */}
      <section className="py-12 bg-mist-50 dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7">
            <div>
              <span className="eyebrow mb-1.5">Médicos disponibles</span>
              <h2 className="font-display text-xl lg:text-2xl font-semibold text-ink-900 dark:text-slate-100 tracking-tight">
                Más de 100 especialistas listos para atenderte
              </h2>
              <p className="text-sm text-ink-500 mt-0.5">Selecciona un médico para ver su perfil completo y disponibilidad.</p>
            </div>
            <div className="flex gap-2">
              <button className="flex items-center gap-2 px-4 py-2 rounded-xl border border-clinical-200 dark:border-slate-700 text-sm text-ink-700 dark:text-slate-400 hover:bg-clinical-50 dark:hover:bg-slate-800 transition-colors">
                <MapPin className="h-4 w-4" /> Ver en mapa
              </button>
              <button onClick={() => setShowFilters(!showFilters)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl border border-clinical-200 dark:border-slate-700 text-sm text-ink-700 dark:text-slate-400 hover:bg-clinical-50 dark:hover:bg-slate-800 transition-colors">
            ...
            {isLoading ? [...Array(8)].map((_, i) => <SkeletonCard key={i} />) :
              (doctors as unknown as any[]).map((doc: any) => (
                <motion.div key={doc.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  className="bg-white dark:bg-slate-900 rounded-2xl border border-mist-200 dark:border-slate-800 p-4 hover:shadow-clinical-sm hover:-translate-y-0.5 transition-all">
                  {/* Availability + fav */}
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => toggleFav(doc.id)} className="p-1 rounded-full hover:bg-mist-100 dark:hover:bg-slate-800 transition-colors">
                      <Heart className={cn('h-4 w-4 transition-colors', favorites.has(doc.id) ? 'fill-vital-500 text-vital-500' : 'text-slate-300')} />
                    </button>
                  </div>
                  {/* Doctor info */}
                  <div className="flex items-start gap-3 mb-3">
                    <div className="relative shrink-0">
                      <img src={doc.avatar} alt={doc.name}
                        className="w-14 h-14 rounded-xl object-cover bg-clinical-50"
                        onError={e => { (e.target as HTMLImageElement).src = `https://api.dicebear.com/7.x/personas/svg?seed=${doc.id}`; }} />
                      {doc.isVerified !== false && (
                        <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-clinical-700 flex items-center justify-center">
                          <CheckCircle2 className="h-3 w-3 text-white" />
                        </div>
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-ink-900 dark:text-slate-100 text-sm leading-tight">{doc.name}</p>
                      <p className="text-clinical-700 dark:text-clinical-300 text-xs font-medium mt-0.5">{doc.specialty}</p>
                      <p className="text-ink-500/70 text-xs flex items-center gap-1 mt-0.5">
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className={cn('h-3 w-3', i < Math.round(doc.rating) ? 'fill-vital-500 text-vital-500' : 'fill-transparent text-slate-200 dark:text-slate-700')} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                    ))}
                    <span className="text-xs text-ink-500">({doc.reviewCount || doc.reviews || 0})</span>
                  </div>
                  {/* Modalities */}
                  <div className="flex gap-1.5 mb-4 flex-wrap">
                    {['In-Person', 'Video', 'Chat'].map(m => (
                      <span key={m} className="text-xs px-2 py-0.5 rounded-full border border-mist-200 dark:border-slate-700 text-ink-500 dark:text-slate-400">{m}</span>
                    ))}
                  </div>
                  {/* Price + CTA */}
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-ink-500/70">Desde</p>
                      <p className="font-bold text-ink-900 dark:text-slate-100 text-base">S/ {doc.price}</p>
                    </div>
                    <Link to={`/doctor/${doc.id}`}
                      className="bg-clinical-700 hover:bg-clinical-800 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-colors">
                      Ver perfil
                    </Link>
                  </div>
                </motion.div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="w-9 h-9 rounded-xl border border-mist-200 dark:border-slate-700 flex items-center justify-center disabled:opacity-40 hover:bg-mist-100 dark:hover:bg-slate-800 transition-colors">
                <ChevronLeft className="h-4 w-4" />
              </button>
              {[...Array(Math.min(totalPages, 10))].map((_, i) => (
                <button key={i} onClick={() => setPage(i + 1)}
                  className={cn('w-9 h-9 rounded-xl text-sm font-medium transition-colors',
                    page === i + 1 ? 'bg-clinical-700 text-white' : 'border border-mist-200 dark:border-slate-700 text-ink-700 dark:text-slate-400 hover:bg-mist-100 dark:hover:bg-slate-800')}>
                  {i + 1}
                </button>
              ))}
              {totalPages > 10 && <span className="text-ink-500/60">...</span>}
              {totalPages > 10 && (
                <button onClick={() => setPage(totalPages)}
                  className="w-9 h-9 rounded-xl border border-mist-200 dark:border-slate-700 text-sm font-medium text-ink-700 dark:text-slate-400 hover:bg-mist-100 dark:hover:bg-slate-800 transition-colors">
                  {totalPages}
                </button>
              )}
          ...
          {/* Trust bar */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-12 pt-8 border-t border-mist-200 dark:border-slate-800">
            {TRUST_ITEMS.map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-500 dark:text-slate-400 mt-0.5">{item.d}</p>
                </div>
              </div>
            ))}
          </div>
